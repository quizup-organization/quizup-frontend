...
import type {AnswerQuestionRequest, CreateBotGameRequest, GameNotification, GameResponse,} from '../types/game.types'
...
export const getGameById = (gameId: string) =>
    axiosInstance.get<GameResponse>(`${GAME_API_BASE}/games/${gameId}`)
...
export const joinGame = (gameId: string) =>
    axiosInstance
        .post<void>(`${GAME_API_BASE}/games/${gameId}/join`)
...
export const cancelGame = (gameId: string, reason?: string) =>
    axiosInstance.post<void>(`${GAME_API_BASE}/games/${gameId}/cancel`, null, {params: {reason}})

export const searchGames = (body: SearchRequest) =>
    axiosInstance.post<PageResponse<GameResponse>>(`${GAME_API_BASE}/games/search`, body)
        .then((response) => response.data)
